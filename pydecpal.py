...
def snes_palette_to_rgba(palette_data: bytes) -> list[tuple[int, int, int, int]]:
    """Converts SNES BGR555 palette data to a list of RGBA tuples."""
    rgba_palette = []
    # MODIFIED: Removed the min(len(palette_data), 32) limit to process the entire data.
...
# MODIFIED: This function now creates a grid for better visualization of large palettes.
def save_palette_as_png(palette: list, output_filename: str, swatch_size: int = 16, colors_per_row: int = 16):
    """Saves a palette (list of RGBA tuples) as a PNG image, arranged in a grid."""
    if not palette:
        print(f"    Warning: Empty palette. Skipping save for '{output_filename}'.")
        return
    ...
    try:
        ...
        img_width = colors_per_row * swatch_size
        ...
        img.save(output_filename)
        print(f"  -> Saved palette to '{output_filename}'")
    except Exception as e:
        print(f"  -> Error saving palette PNG '{output_filename}': {e}")
...
def main():
    ...
    rom_path = sys.argv[1]
    graphics_output_dir = "decompressed_pngs_color"
    palette_output_dir = "decompressed_palettes"
    ...
    with open(rom_path, 'rb') as f:
        for hex_str in offsets_hex:
            try:
                ...
                # The classification logic remains the same. 256 bytes fits this rule.
                if 0 < len(data) < 512 and len(data) % 2 == 0:
                    palettes[hex_str] = snes_palette_to_rgba(data)
                    print(f"  0x{hex_str}: Identified as PALETTE (Compressed: {compressed_size} bytes -> Decompressed: {len(data)} bytes, {len(palettes[hex_str])} colors)")
                elif len(data) >= 512:
    ...
    if not graphics:
        print("No graphics files were successfully decompressed. Exiting graphics processing.")
    else:
        ...
        for hex_str, graphic_data in sorted(graphics.items()):
            ...
            if best_palette_key:
                ...
                print(f"  -> Applying palette from 0x{best_palette_key} ({len(palette_to_use)} colors)")
            else:
                print("  -> No preceding palette found. Using grayscale fallback.")

            output_path = os.path.join(graphics_output_dir, f"graphic_{hex_str}.png")
            save_snes_4bpp_as_png(graphic_data, output_path, palette_to_use, tiles_per_row=32)
    
    # --- PASS 3: Save the palettes themselves as images ---
    print("\n--- PASS 3: Generating Palette PNGs ---")
    if not palettes:
        print("No palettes were successfully decompressed.")
    else:
        for hex_str, palette_data in sorted(palettes.items()):
            print(f"Processing palette 0x{hex_str}:")
            output_path = os.path.join(palette_output_dir, f"palette_{hex_str}.png")
            save_palette_as_png(palette_data, output_path)

    print("\n-------------------------------------")
    print(f"Batch processing complete. Check the '{graphics_output_dir}' and '{palette_output_dir}' folders.")
